		...
		case 0x0297228f /* "pursue" */:
			{
				...
				if (child->QueryFloatAttribute("angle", &mPursue.mOffset.a) == TIXML_SUCCESS)
					mPursue.mOffset.a *= float(M_PI) / 180.0f;
				child->QueryFloatAttribute("x", &mPursue.mOffset.p.x);
				child->QueryFloatAttribute("y", &mPursue.mOffset.p.y);
			}
			break;
...
void Aimer::Pursue(float aStep, Entity *entity, const AimerTemplate &aimer, Entity *targetEntity)
{
	...
	// apply pursuit offset (HACK)
	targetDir = aimer.mPursue.mOffset.Untransform(targetDir);
