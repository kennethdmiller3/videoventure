...
#include "Behavior/RipOffBehavior.h"
...
bool AimerTemplate::Configure(const TiXmlElement *element, unsigned int aId)
{
	for (const TiXmlElement *child = element->FirstChildElement(); child != NULL; child = child->NextSiblingElement())
	{
		...
		case 0xc4fc7791 /* "ripoff" */:
			{
				RipOffBehaviorTemplate &ripoffbehavior = Database::ripoffbehaviortemplate.Open(aId);
				ripoffbehavior.Configure(child, aId);
...
, Brain()
{
	...
	const RipOffBehaviorTemplate &ripoffbehavior = Database::ripoffbehaviortemplate.Get(aId);
	if (ripoffbehavior.mTargetTag)
	{
		mBehaviors.push_back(new RipOffBehavior(aId, this));
		mScheduler.Run(*mBehaviors.back());
	}
}
