...
static const size_t jointsize = 
	...
	sizeof(b2LineJointDef))
#else
	sizeof(b2PulleyJointDef)
#endif
	)));
...
namespace Database
{
...
#ifdef B2_JOINE_LINE_H
	Typed<Typed<b2LineJointDef> > collidabletemplateline(0xc4e93aba /* "collidabletemplateline" */);
#endif
#endif
	...
	Typed<Collidable::BoundarySignal> collidableboundaryviolation(0x1544aaaf /* "collidableboundaryviolation" */);
...
#ifdef B2_LINE_JOINT_H
		case e_lineJoint:		jointdef = new(jointpool.malloc()) b2LineJointDef(*static_cast<const b2LineJointDef *>(jointsource)); break;
#endif
...
bool CollidableTemplate::Configure(const TiXmlElement *element, unsigned int id)
{
	...
	for (const TiXmlElement *child = element->FirstChildElement(); child != NULL; child = child->NextSiblingElement())
	{
		...
		case 0xa59c5ee9 /* "linejoint" */:
			{
#ifdef COLLIDABLE_JOINT_DATABASE
				Database::Typed<b2LineJointDef> &joints = Database::collidabletemplateline.Open(id);
				unsigned int jointid = joints.GetCount()+1;
				b2LineJointDef &joint = joints.Open(jointid);
				CollidableTemplate::ConfigureLineJoint(child, joint);
				joints.Close(jointid);
				Database::collidabletemplaterevolute.Close(id);
#else
				b2LineJointDef *joint = new(jointpool.malloc()) b2LineJointDef();
				if (const char *name = child->Attribute("name"))
					joints.Put(Hash(name), joint);
				else
					joints.Put(joints.GetCount() + 1, joint);
...
public:
	...
	virtual void Add(const b2ContactPoint* point)
	{
		...
		Database::collidablecontactadd.Get(id1)(id1, id2, 0.0f /*shape1->GetBody()->m_sweep.t0*/, *point);
		Database::collidablecontactadd.Get(id2)(id2, id1, 0.0f /*shape2->GetBody()->m_sweep.t0*/, *point);
	};
	...
	virtual void Remove(const b2ContactPoint* point)
	{
		...
		Database::Key id1 = reinterpret_cast<Database::Key>(shape1->GetUserData());
		Database::Key id2 = reinterpret_cast<Database::Key>(shape2->GetUserData());
		Database::collidablecontactremove.Get(id1)(id1, id2, 0.0f /*shape1->GetBody()->m_sweep.t0*/, *point);
		Database::collidablecontactremove.Get(id2)(id2, id1, 0.0f /*shape2->GetBody()->m_sweep.t0*/, *point);
	}
...
public:
	...
	virtual void Violation(b2Body* body)
	{
		...
		Database::collidableboundaryviolation.Get(id)(id);
		if (mList.empty())
			Activate();
		mList.push_back(id);
	}
...
#ifdef B2_LINE_JOINT_H
	const Database::Typed<b2LineJointDef> &lines = Database::collidabletemplateline.Get(id);
	for (Database::Typed<b2LineJointDef>::Iterator lineitor(&lines); lineitor.IsValid(); ++lineitor)
		CreateJoint(lineitor.GetValue());
#endif
#else
	// for each joint...
