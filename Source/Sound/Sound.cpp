...
bool SoundTemplate::ConfigureFile(const TiXmlElement *element, unsigned int id)
{
	...
	mSize = mLength * sizeof(short) + info.length * ((info.flags & BASS_SAMPLE_8BITS) ? sizeof(short) : 1) / info.chans;
	mData = realloc(mData, mSize);

	// set frequency
	mFrequency = info.freq;
	...
	if ((info.chans > 1) || (info.flags & BASS_SAMPLE_8BITS))
	{
		...
		{
			for (unsigned int in = 0, out = mLength, samp = 0; in < info.length; ++in)
			{
				accum += static_cast<unsigned char *>(buf)[in];
				if (++samp >= info.chans)
				{
					static_cast<short *>(mData)[out++] = short(accum * 257 / samp - 32768);
					accum = 0;
					samp = 0;
				}
			}
		...
		{
			for (unsigned int in = 0, out = mLength, samp = 0; in < info.length / sizeof(short); ++in)
			{
				accum += static_cast<short *>(buf)[in];
				if (++samp >= info.chans)
				{
					static_cast<short *>(mData)[out++] = short(accum / samp);
					accum = 0;
					samp = 0;
				}
			}
	...
	{
		// copy sound data
		BASS_SampleGetData(handle, static_cast<short *>(mData) + mLength);
	}
