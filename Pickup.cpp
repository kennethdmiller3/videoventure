...
public:
	...
	void Update(float aStep)
	{
		...
		for (Database::Typed<float>::Iterator itor(Database::pickupresource.Get(mId).Find(aHitTeam)); itor.IsValid(); ++itor)
		{
			...
			{
				// add the resource
				resource->Add(mId, itor.GetValue());
			}
		}
...
void Pickup::Collide(unsigned int aId, unsigned int aHitId, float aFraction, const b2ContactPoint &aPoint)
{
	...
	{
		// get the pickup link template
		const LinkTemplate &link = Database::pickuplink.Get(mId).Get(aHitTeam).Get(itor.GetKey());

		// if the pickup links an item...
		if (link.mSecondary)
		{
			mDestroy = true;
			break;
	...
	// for each resource template...
	for (Database::Typed<ResourceTemplate>::Iterator itor(Database::resourcetemplate.Find(aHitId)); itor.IsValid(); ++itor)
	{
		// get the pickup resource template
		const float &resource = Database::pickupresource.Get(mId).Get(aHitTeam).Get(itor.GetKey());

		// if the pickup gives a resource...
		if (resource)
		{
			mDestroy = true;
			break;
